...
import { Ticket, ShoppingBag, ChevronRight, Gift, Clock, QrCode } from "lucide-react";
...
export default function PassScreen({ user, db, onPay, onRequestCash }) {
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where("status", "in", ["created", "scanned", "cash", "paid", "reward_pending"]),
      ...
      <div>
        ...
        {regularOrders.length === 0 ? (
          <div className="text-center py-10 bg-white rounded-3xl border border-gray-100 border-dashed">
            <div className="text-gray-300 mb-2"><ShoppingBag size={48} className="mx-auto"/></div>
            <p className="text-gray-400 text-sm font-medium">Aucune commande active.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {regularOrders.map(order => (
              ...
              >
                <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${order.status === 'paid' ? 'bg-green-500' : 'bg-teal-500'}`} />
                <div className="text-left pl-2">
                  ...
                  <div className="text-xs text-gray-500 flex items-center gap-1">
                    <Clock size={12} /> 
                    {order.items?.length} article(s) • <span className="uppercase font-bold">{order.status}</span>
                  </div>
                </div>
